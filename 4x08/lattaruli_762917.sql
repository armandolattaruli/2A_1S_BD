...
-- query 70
SELECT testo FROM inserzioni
    JOIN categorie ON inserzioni.categoria = categorie.idCategoria
    WHERE categorie.categoriaPadre IS NULL;

-- query 71
SELECT count(codice) AS "Numero inserzioni categorie principali: " FROM inserzioni
    JOIN categorie ON inserzioni.categoria = categorie.idCategoria
    WHERE categorie.categoriaPadre IS NULL;

-- query 72
-- modifica effettuata sopra

-- query 73
UPDATE insaz 
    set costo=30
    WHERE idInserzione LIKE "000011";

UPDATE insaz 
    set costo=31
    WHERE idInserzione LIKE "000012";

UPDATE insaz 
    set costo=32
    WHERE idInserzione LIKE "000015";

UPDATE insaz 
    set costo=33
    WHERE idInserzione LIKE "000019";

UPDATE insaz 
    set costo=34
    WHERE idInserzione LIKE "000013";

UPDATE insaz 
    set costo=35
    WHERE idInserzione LIKE "000016";

UPDATE insaz 
    set costo=36
    WHERE idInserzione LIKE "000020";

UPDATE insaz 
    set costo=37
    WHERE idInserzione LIKE "000014";

UPDATE insaz 
    set costo=45
    WHERE idInserzione LIKE "000017";

UPDATE insaz 
    set costo=49
    WHERE idInserzione LIKE "000018";

-- query 74
SELECT SUM(costo) AS "Costo totale per l'azienda con codice COM000: " FROM insaz WHERE idAzienda = "COM000";

-- query 75
SELECT * FROM inserzioni;

-- query 76
SELECT count(codice) AS "Totale articoli pubblicati: " FROM inserzioni;

-- query 77
SELECT * FROM inserzioni WHERE testo LIKE "%affa%";

-- query 78
SELECT count(codice) AS "Articoli che contengo la stringa 'affa': " FROM inserzioni WHERE testo LIKE "%affa%";

-- query 79
SELECT idInserzione, costo FROM insaz
UNION
SELECT idInserzione, costo FROM inspriv;

-- query 80
SELECT count(idInserzione) FROM insaz
UNION ALL
SELECT count(idInserzione) FROM inspriv;

-- query 81
SELECT(
    (SELECT COUNT(idInserzione) FROM insaz) + (SELECT COUNT(idInserzione) FROM inspriv)
) AS "Totale inserzioni presenti in inspriv e insaz: ";


-- query 82
SELECT COUNT(*) AS "Numero per categoria:", categoria from inserzioni GROUP BY categoria;

-- query 83
SELECT nomecategoria AS 'Categorie con più di un inserzione'
...
-- query 84
SELECT nomecategoria, count(codice) AS "Numero annunci per categoria"
FROM inserzioni i JOIN categorie c ON i.categoria=c.idCategoria
GROUP BY categoria;

-- query 85
SELECT count(idinserzione) AS "Numero annunci per testata"
FROM  instest
GROUP BY idtestata;

-- query 86
SELECT nome, count(idinserzione) AS "Numero annunci per testata"
FROM  instest i NATURAL JOIN testate
GROUP BY idtestata;

-- query 87
SELECT codice, testo, categoria
FROM  inserzioni i JOIN insaz az ON i.codice=az.idinserzione
WHERE costo < 35;
...
-- query 89
SELECT count(idPrivato)
FROM  privati p JOIN citta c ON p.citta=c.cap
